...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnGUI ()
	{
        ...
        GUILayout.BeginHorizontal();
        NGUISettings.compressAfterTrueColor = EditorGUILayout.Toggle("compressAfterTrueColor", NGUISettings.compressAfterTrueColor, GUILayout.Width(100f));
        GUILayout.Label("forceARGB32后转压缩纹理", GUILayout.MinWidth(70f));
        GUILayout.EndHorizontal();
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
                ...
                if (delete)
                {
                    ...
                    UpdateAtlas(NGUISettings.atlas, sprites);
                    if (NGUISettings.compressAfterTrueColor)
                    {
                        //AtlasHelper.ConvertAtlasToCompress(NGUISettings.atlas);
                    }
                    mDelNames.Clear();
